...
function getTier(solved) {
  ...
  if (solved >= 5)
    return {
      name: "ROOKIE",
      color: "#888aaa",
      glow: "rgba(136,138,170,0.3)",
      icon: "🌱",
    };
  return {
    name: "NEWBIE",
    color: "#444466",
    glow: "rgba(68,68,102,0.2)",
    icon: "🐣",
  };
}
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      >
        ...
        <span style={{ fontFamily: "'Montserrat'", fontSize: 12, color }}>
          {count}
        </span>
      </div>
      <div
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        "https://api.cloudinary.com/v1_1/dg4s9fydz/image/upload",
        { method: "POST", body: fd },
      );
            ...
            <div>
              {config.type === "textarea" ? (
                <textarea
                  ...
                  onChange={(e) =>
                    setForm({ ...form, [activeField]: e.target.value })
                  }
              ...
              >
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (!error) setActiveField(null);
                  }}
                  style={{
                    background: saving ? "#1a1a2e" : "#00ff88",
                    color: saving ? "#555" : "#050508",
                    border: "none",
                    padding: "9px 20px",
                    borderRadius: 7,
              ...
              {[
                {
                  ...
                  items: fieldsConfig.general,
                },
                {
                  title: "Experience",
                  desc: "Your background and expertise.",
                  items: fieldsConfig.experience,
                },
                  ...
                  >
                    {items.map((item, idx) => (
                      <div key={item.key}>
                        ...
                        {idx < items.length - 1 && (
                          <div style={{ height: 1, background: "#1e1e30" }} />
                        )}
                      </div>
                    ))}
                  </div>
...
export default function ProfileDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const subs = [];
        for (const p of (Array.isArray(solvedList) ? solvedList : []).slice(
          0,
          8,
        )) {
          try {
            const r = await axiosClient.get(
              `/problem/submittedProblem/${p._id}`,
            );
            const list = r.data?.data || r.data || [];
            if (Array.isArray(list))
              subs.push(...list.map((s) => ({ ...s, problemTitle: p.title })));
          } catch {}
        }
  ...
  const acceptRate =
    submissions.length > 0
      ? Math.round((acceptedSubs / submissions.length) * 100)
      : 0;
  const topicMap = {};
  solved.forEach((p) => {
            ...
            <div style={{ ...card(), textAlign: "center" }}>
              ...
              <div
                style={{
                  ...
                  overflow: "hidden",
                  margin: "0 auto 14px",
                  background: "linear-gradient(135deg,#1e2e44,#0a1a12)",
              ...
              <button
                ...
                onMouseEnter={(e) => {
                  e.target.style.boxShadow = "0 0 30px rgba(0,255,136,0.45)";
                }}
                onMouseLeave={(e) => {
                  e.target.style.boxShadow = "0 0 20px rgba(0,255,136,0.25)";
                }}
              >
                Edit Profile
              </button>
                    ...
                    .map((skill, i) => (
                      <span
                        ...
                        style={{
                          ...
                          padding: "4px 10px",
                          borderRadius: 6,
                          background: `${topicColors[i % topicColors.length]}15`,
                          border: `1px solid ${topicColors[i % topicColors.length]}30`,
            ...
            >
              ...
              >
                <Ring
                  ...
                  label="Easy"
                  sublabel={
                    easyTotal ? Math.round((easySolved / easyTotal) * 100) : 0
                  }
                />
                <Ring
                  value={medSolved}
                  ...
                  label="Med"
                  sublabel={
                    medTotal ? Math.round((medSolved / medTotal) * 100) : 0
                  }
                />
                <Ring
                  value={hardSolved}
                  ...
                  label="Hard"
                  sublabel={
                    hardTotal ? Math.round((hardSolved / hardTotal) * 100) : 0
                  }
                />
              </div>
                  ...
                  {["overview", "submissions", "topics"].map((tab) => (
                    <button
                      ...
                      style={{
                        ...
                        padding: "8px 14px",
                        textTransform: "capitalize",
                        color: activeTab === tab ? "#00ff88" : "#555577",
                        borderBottom:
                          activeTab === tab
                            ? "2px solid #00ff88"
                ...
                {activeTab === "topics" && (
                  <div>
                    {Object.keys(topicMap).length === 0 ? (
                      ...
                      >
                        Solve problems to see topic stats.
                      </div>
                    ) : (
                  ...
                  {solved.slice(0, 9).map((p) => (
                    ...
                    >
                      <div
                        ...
                        onMouseLeave={(e) => {
                          e.currentTarget.style.borderColor = "#1e1e30";
                          e.currentTarget.style.background = "#050508";
                        }}
                      >
                        ...
                        <span
                          style={{
                            ...
                            marginLeft: 8,
                            ...(p.difficulty === "easy"
                              ? { background: "#0a2e1a", color: "#00ff88" }
                              : p.difficulty === "medium"
                                ? { background: "#2e2200", color: "#ffd166" }
